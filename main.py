...
try:
    ...
    if users:
        ...
        # Baza hali yangi bo'lsa va ba'zi ustunlar yo'q bo'lsa xato bermasligi uchun tekshiramiz
...
# --- 5. BOT RUNNER (KILLER, SINGLETON, THREAD) ---
def run_bot_in_background():
    """Botni alohida oqimda va xavfsiz ishga tushirish"""
    
    async def _runner():
        try:
            # 1. KILLER: Eski webhooklarni o'chirish (Server qotmasligi va bot "spam" bo'lmasligi uchun)
    ...
    def _thread_target():
        # Yangi event loop ochamiz (Streamlit bilan to'qnashmasligi uchun)
    ...
    if not is_running:
        # Daemon=True orqali Streamlit yopilsa, bot ham avtomatik yopiladi
